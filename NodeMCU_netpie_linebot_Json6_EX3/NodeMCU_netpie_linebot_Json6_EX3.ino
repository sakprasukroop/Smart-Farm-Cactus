...
const char* ssid     = "nomsod1694"; //const char* ssid     = "Surasak";        
const char* password = "sak7801244"; //const char* password = "0875318445";    

//const char* ssid     = "STK_SERVICE"; //const char* ssid     = "Surasak";        
//const char* password = "81704890"; //const char* password = "0875318445";    
...
void onMsghandler(char *topic, uint8_t* msg, unsigned int msglen) 
{
    ...
    if ( msgLINE == "สวัสดี" ) send_json("สวัสดี ผมชื่อจ่าวิทย์ ยินดีรับใช้ครับ แนะนำให้พิมพ์ Lamp หรือ Analog หรือ Show");
     ...
     else if ( msgLINE == "Show" ) show_data();
         ...
         if (find_text == "-1") 
           ...
           else 
           {
            // แสดงว่ามีข้อความที่ค้นหา
              ...
              EEPROM.write(0,setpoint);          // เก็บลง EEPROM address 0
              EEPROM.commit();
...
void show_data()
{
   ...
   /*
    ...
    // ตรวจสอบสถานะหลอดไฟ
    if (digitalRead(led1)== LOW) send_json("สถานะหลอดไฟ เปิดอยู่ครับ (LED ON)");
        else if (digitalRead(led1)== HIGH) send_json("สถานะหลอดไฟ ปิดอยู่ครับ (LED OFF)");
        
    // ตรวจสอบอนาล็อก 
    String text = "ค่าอนาล็อกขณะนี้มีค่าเท่ากับ : " + String(AnalogValue) + "  ครับผม";
    send_json(text);

    text = "การตั้งค่าอนาล็อกขณะนี้อยู่ที่ มากกว่าหรือเท่ากับ : " + String(setpoint) + "  led2 ทำงาน ครับผม";
    send_json(text);

    if (digitalRead(led2)== LOW) send_json("ขณะนี้สถานะ led2 ทำงานอยู่ครับ");
        else if (digitalRead(led2)== HIGH) send_json("ขณะนี้สถานะ led2 ไม่ทำงานครับ");
